...
export default function Testimonials() {
  return (
    <div className=" py-24 sm:py-32 bg-[#fcf9f3] rounded-2xl">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-base/7 font-semibold text-black-400 text-center">Our clients are at the heart of everything we do. Here’s what they have to say about their experience with us.</h2>
          <p className="mt-2 text-4xl font-semibold  tracking-tight text-pretty  py-4 text-black text-center sm:text-5xl">
            ...
            {testimonials.map((testimonial) => (
              <div key={testimonial.author.handle} className="pt-8 sm:inline-block sm:w-full sm:px-4">
                <figure className="rounded-2xl bg-white/2.5 p-8 text-sm/6">
                  <blockquote className="text-black-100">
                    <p>{`“${testimonial.body}”`}</p>
                  </blockquote>
                  <figcaption className="mt-6 flex items-center gap-x-4">
                    <img alt="" src={testimonial.author.imageUrl} className="size-10 rounded-full bg-gray-800" />
                    <div>
                      <div className="font-semibold text-black">{testimonial.author.name}</div>
                      <div className="text-black-400">{`@${testimonial.author.handle}`}</div>
                    </div>
                  </figcaption>
                </figure>
              </div>
